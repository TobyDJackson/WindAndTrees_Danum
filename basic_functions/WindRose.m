...
color = DrawPatches(N,n,vwinds,count,colorfun,figcolor,min_radius,colors,inverse); % Draw the windrose, knowing the angles, the range for each direction, the speed ranges, the count (frequency) values, the colormap used and the colors used.

axis off;
axis equal;
circlemax = circlemax/max(eps,scalefactor);
...
xlim([-radiusmax radiusmax]/scalefactor);
ylim([-radiusmax radiusmax]/scalefactor);
...
elseif LegendType==1
    ...
    colormap(interp1(vwinds,color,linspace(min(vwinds),max(vwinds),256)));
...
function color = DrawPatches(N,n,vwinds,count,colorfun,figcolor,min_radius,colors,inverse)
    if isempty(colors)
    ...
    if inverse                                                             % If wind speeds are shown in inverse way (slowest is outside)
        ...
        count          = cumsum(fliplr(count),2);                          % Cumsum inverting.
    end
    for i=1:length(N)
...
function leyenda = CreateLegend(vwinds,lablegend,legendvariable,inverse)
    ...
    for j=orden                                                            % Cross the speeds in the specified direction
        ...
        if j==length(vwinds)                                               % When last index is reached
            ...
            string = sprintf('%g %s %s < %g',round(vwinds(j),1),'\leq',legendvariable,round(vwinds(j+1),1)); % Set v1 <= v2 < v1
        end
